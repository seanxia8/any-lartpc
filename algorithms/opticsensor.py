...
def PMT_angle_general(x: torch.Tensor, **kwargs):
    ...
    #efficiency = 1 - 1 / (1 + torch.exp(-sigmoid_coeff * 180./np.pi * (torch.abs(np.pi/2 - x) - np.pi/4)))
    efficiency = 1 / (1 + torch.exp(-sigmoid_coeff * 180. / np.pi * (torch.abs(np.pi / 2 - x) - np.pi / 4)))
    return efficiency
...
class dot_steel_PMT():
    ...
    def compute_pmt_eff(self, angle: torch.Tensor, n_photons: torch.Tensor, **kwargs):
        ...
        n_photons = n_photons.to(self.pmt_eff.device)
        self.n_PE = (n_photons * self.pmt_eff).detach().cpu().int()
